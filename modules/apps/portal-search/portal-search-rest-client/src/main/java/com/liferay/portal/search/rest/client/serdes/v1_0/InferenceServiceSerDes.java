...
public class InferenceServiceSerDes {
	...
	public static String toJSON(InferenceService inferenceService) {
		...
		if (inferenceService.getInferenceServiceFields() != null) {
			if (sb.length() > 1) {
				sb.append(", ");
			}
			...
			for (int i = 0;
				 i < inferenceService.getInferenceServiceFields().length; i++) {

				sb.append(
					String.valueOf(
						inferenceService.getInferenceServiceFields()[i]));

				if ((i + 1) <
						inferenceService.getInferenceServiceFields().length) {
		...
		if (inferenceService.getName() != null) {
			if (sb.length() > 1) {
				sb.append(", ");
			}
			...
			sb.append(_escape(inferenceService.getName()));
		...
		if (inferenceService.getTaskTypes() != null) {
			if (sb.length() > 1) {
				sb.append(", ");
			}
			...
			for (int i = 0; i < inferenceService.getTaskTypes().length; i++) {
				sb.append(_toJSON(inferenceService.getTaskTypes()[i]));

				if ((i + 1) < inferenceService.getTaskTypes().length) {
					sb.append(", ");
				}
			}
	...
	public static Map<String, String> toMap(InferenceService inferenceService) {
		...
		if (inferenceService.getInferenceServiceFields() == null) {
			map.put("inferenceServiceFields", null);
		}
		else {
			map.put(
				"inferenceServiceFields",
				String.valueOf(inferenceService.getInferenceServiceFields()));
		}
		...
		if (inferenceService.getName() == null) {
			...
			map.put("name", String.valueOf(inferenceService.getName()));
		}
		...
		if (inferenceService.getTaskTypes() == null) {
			map.put("taskTypes", null);
		}
		else {
			map.put(
				"taskTypes", String.valueOf(inferenceService.getTaskTypes()));
		}
	...
	public static class InferenceServiceJSONParser
		...
		protected boolean parseMaps(String jsonParserFieldName) {
			if (Objects.equals(jsonParserFieldName, "inferenceServiceFields")) {
				return false;
			}
			...
			else if (Objects.equals(jsonParserFieldName, "taskTypes")) {
				return false;
			}
		...
		protected void setField(
			...
			if (Objects.equals(jsonParserFieldName, "inferenceServiceFields")) {
				if (jsonParserFieldValue != null) {
					...
					InferenceServiceField[] inferenceServiceFieldsArray =
						new InferenceServiceField[jsonParserFieldValues.length];

					for (int i = 0; i < inferenceServiceFieldsArray.length;
						 i++) {

						inferenceServiceFieldsArray[i] =
							InferenceServiceFieldSerDes.toDTO(
								(String)jsonParserFieldValues[i]);
					}

					inferenceService.setInferenceServiceFields(
						inferenceServiceFieldsArray);
				}
			}
			else if (Objects.equals(jsonParserFieldName, "name")) {
				if (jsonParserFieldValue != null) {
					inferenceService.setName((String)jsonParserFieldValue);
				}
			}
			else if (Objects.equals(jsonParserFieldName, "service")) {
				if (jsonParserFieldValue != null) {
...
// LIFERAY-REST-BUILDER-HASH:814209047